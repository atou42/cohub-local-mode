...
修复、本地回归和生产部署已经完成。问题已由用户提供的 iOS 实机截图确认。输入结束且键盘收起后，应用主体停留在较小高度，底部露出约一百个 CSS 像素的白色页面背景；同一页面的正常截图没有该空白。当前机器没有 Xcode 和 iOS Simulator，仍需 iOS 实机复测后才能完成最终验收。
...
- 生产页面绕过缓存重载后，模拟键盘把可视高度从 852 压缩到 744 像素，失焦并恢复视口后外壳回到 852 像素；页面没有脚本异常。
